  ...
  template: `
    <div class="max-w-lg">
      <div class="panel !p-0 overflow-hidden">
        <div class="title-bar rounded-none border-x-0 border-t-0">{{ isEdit ? 'Edit Practice' : 'New Speaking Practice' }}</div>
        <form class="space-y-3 p-4 text-sm" [formGroup]="form" (ngSubmit)="submit()">
          ...
          <div>
            <label class="mb-1 block">Prompt / Question</label>
            <textarea class="input-field min-h-[80px]" formControlName="prompt"></textarea>
          </div>
          <div>
            <label class="mb-1 block">Your Response</label>
            <textarea class="input-field min-h-[120px]" formControlName="response"></textarea>
          </div>
          <div>
