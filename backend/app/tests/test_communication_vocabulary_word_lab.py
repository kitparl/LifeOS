...
async def test_save_bookmarks_the_word_for_the_user_even_when_it_already_existed(client, wordnik):
    await _seed(client, 3)
    h = await _auth(client, "wlsavebm@example.com")
    other = await _auth(client, "wlsavebmother@example.com")
    await _connect(client, h)

    new = (await client.post(f"{VOCAB}/word-lab/save", headers=h, json={"term": "herald", "definition": "x"})).json()
    existing = (await client.post(f"{VOCAB}/word-lab/save", headers=h, json={"term": "Word-1", "definition": "x"})).json()
    again = await client.post(f"{VOCAB}/word-lab/save", headers=h, json={"term": "HERALD", "definition": "y"})
    assert again.status_code == 200

    bookmarks = (await client.get(f"{VOCAB}/bookmarks", headers=h)).json()
    ids = [b["vocabulary"]["id"] for b in bookmarks["items"]]
    assert sorted(ids) == sorted([new["id"], existing["id"]])  # once each, no duplicates
    assert existing["id"] == "v000001"
    assert (await client.get(f"{VOCAB}/bookmarks", headers=other)).json()["total"] == 0
