...
- **Word Lab tab** (`components/vocabulary-word-lab.component.ts`):
  - One search box plus tool chips: Dictionary, Synonyms, Explorer (reverse dictionary: describe an idea and get words), Rhymes, and Game (Guess the word / Guess the meaning / Scramble). Below the tools, **Your saved words** lists the user's bookmarks, newest first.
...
- **Save as vocabulary** inserts a row into the same `vocabulary` table (`source="user_saved"`, id `x` + 15 hex chars). If the term already exists, case-insensitively, the existing id is returned instead. Either way, the word is also **bookmarked for the user**: `vocabulary` rows are shared, so the bookmark is the per-user record of "I saved this". That is why existing (for example, seeded) words appear under Your saved words too. Saved rows open in the normal detail page and appear in Library search.
